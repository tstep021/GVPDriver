...
/** 
 * @file         ConfigNotActive.java
 * 
 * @description  This servlet plays message and hangs up
 * 
...
public class ConfigNotActiveMsg extends BaseCallFlow
{	
	private static final long serialVersionUID = 2765155167172832379L;
	private static final Logger logger = Logger.getLogger(ConfigNotActiveMsg.class);
	...
	public VXML addDocumentBody(VXML vxml, ICallInfo ci, String submitNext, HttpServletRequest request) {
		...
		for(int i=0;i<len;i++) {
			...
			vxml.AudioStart(promptpath + BaseGlobalConfig.languages[i] + "/notActivated.wav");
				vxml.Text(BaseUtils.getStringFromFile(BaseGlobalConfig.resource_repository_basepath + promptpath + BaseGlobalConfig.languages[i] + "/notActivated.txt"));
			vxml.AudioEnd();							
